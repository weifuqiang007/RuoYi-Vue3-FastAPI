...
### 1.2 为文档 Controller 添加路由级角色校验

修改 `module_rag/controller/document_controller.py`，采用与知识库 Controller 相同的 AOP 模式。

需要修改的内容：
1. 在文件顶部增加导入：`from common.aspect.interface_auth import RoleInterfaceAuthDependency`
2. 删除文件内的 `ALLOWED_ROLES` 常量和 `check_role()` 函数
3. 在路由定义的 `dependencies` 中添加 `RoleInterfaceAuthDependency(['admin', 'teacher'])`
4. 删除每个端点函数体内的 `check_role(current_user)` 调用
5. `list`、`download`、`preview` 端点如果不需要 `current_user`，可以移除该参数
...
document_controller = APIRouterPro(
    prefix='/rag/document', order_num=21, tags=['RAG管理-文档'],
...
对 `download_document`、`preview_document` 也做同样处理。

### 1.3 为检索 Controller 添加路由级角色校验
